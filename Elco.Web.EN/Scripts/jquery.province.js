...
function initArea(cfg) {
    ...
    function _initProvince() {
        if (cfg.p.id) {
            var obj = $('#' + cfg.p.id)
            _initOptions(obj, "0").change(function () {
                var index = $(this).get(0).selectedIndex;
                _initCity(index);
            }).val(cfg.p.val);
            _initCity(obj.get(0).selectedIndex);
        }
    }
    function _initCity(selectedIndex) {
        if(selectedIndex === 0 ){return;}
        if (cfg.c.id) {
            var pid = "0_" + (selectedIndex - 1);
            var obj = $('#' + cfg.c.id);
            _initOptions(obj, pid).change(function () {
                var index = $(this).get(0).selectedIndex;
                _initTown(pid,index);
            }).val(cfg.c.val);
            _initTown(pid, obj.get(0).selectedIndex);
        }
    }
    function _initTown(pid,selectedIndex) {
        if (cfg.t && cfg.t.id) {
            //console.info(pid + "_" + (selectedIndex - 1));
            _initOptions($('#' + cfg.t.id), pid+"_" + (selectedIndex - 1));
        }
    }
    function _initOptions(obj, index) {
        var optArray = [],
            opt = '';
        if (obj.length === 0) { return; }
        optArray.push('<option value="">==请选择==</option>');
        $(dsy.Items[index]).each(function () {
            opt = '<option value="' + this + '">' + this + '</option>';
            optArray.push(opt);
        });
        obj.html(optArray.join(''));
        return obj;
    }
    _initProvince();
}
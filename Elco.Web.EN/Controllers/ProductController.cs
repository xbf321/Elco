...
namespace Elco.Web.En.Controllers
{
    public class ProductController : BaseController
    {
        ...
        {
            //设置模板页中变量
            ViewBag.RootCategoryInfo = CategoryService.Get(GeneralConfig.ProductRootId_EN);
        ...
        public ActionResult RenderProductDropDownList(object selectedValue) {
            ...
            var oldCatList = CategoryService.ListAllSubCatById(GeneralConfig.ProductRootId_EN).Where(p => p.IsEnabled == true
                && p.IsDeleted == false);
        ...
        public ActionResult RenderProductMenu() {
            StringBuilder sbHtml = new StringBuilder();
            var allCatList = CategoryService.ListAllSubCatById(GeneralConfig.ProductRootId_EN).Where(p => p.IsDeleted == false && p.IsEnabled == true);
            ...
            fb = (catList, parentId,level,sb) => {
                ...
                string className = parentId == GeneralConfig.ProductRootId_EN ? "class=\"product-menu\"" : string.Empty;

                if (parentId != GeneralConfig.ProductRootId_EN && tempList.Count() > 0)
                {
                    sb.Append("<!--[if lte IE 6]><table><tr><td><![endif]-->");
                }
                sb.AppendFormat("<ul {1}{0}>", string.Empty, parentId == GeneralConfig.ProductRootId_EN ? "id=\"product_menu\"" : string.Format("id=\"p_menu_ul_{0}\"", parentId));
            ...
            fb(allCatList, GeneralConfig.ProductRootId_EN, 0, sbHtml);
